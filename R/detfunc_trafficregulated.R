#' This function gives deterministic realization for n countries with a given regulated strategy
#' @param combinetheta is a matrix of parameters, parameters of each country is on 1 row
#' @param inp is a list include duration : durationtravel (days),
...
#' detfunc_trafficregulated(theta0,inp)

#' #Example2: Realizations of 3 countries
#' P1 = 10^7
#' I1 = 250
#' A1 = 130
#' S1 = P1 - I1 - A1
...
detfunc_trafficregulated =  function(combinetheta, inp){
  ...
  ##############
  numbercountries = nrow(inp$initial_corona)

  status_matrix = matrix(0, nrow = inp$duration, ncol = numbercountries*ncol(inp$initial_corona))
  f_in = matrix(0, nrow =  inp$duration, ncol = numbercountries*ncol(inp$initial_corona))
  f_out = matrix(0, nrow =  inp$duration, ncol = numbercountries*ncol(inp$initial_corona))


  initial = rep(0, numbercountries*ncol(inp$initial_corona))

  for(val3 in 1:numbercountries){
    h1 = 1 + (val3 - 1)*6
    h2 = 6 + (val3 - 1)*6
    initial[h1:h2] = inp$initial_corona[val3,]
